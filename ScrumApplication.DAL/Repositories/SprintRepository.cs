    ...
    public class SprintRepository
    {
        ...
        public static SprintModel GetSprintModel(int projectId)
        {
            using (var db = new ScrumApplicationDbContext())
            {
                ...
                var sprintPTasks = new List<ProjectTask>();
                sprintTasks = sprint.SprintTasks.ToList();
                sprintPTasks = sprint.ProjectTasks.ToList();
                if (sprintTasks != null)
                {
                    var query = from task in sprintTasks
                                orderby task.Done ascending, task.Priority ascending
                                select task;
                    newModel.SprintTasks = query.ToList();
                }
                if (sprintPTasks != null)
                {
                    var nquery = from task in sprintPTasks
                                 ...
                                 select task;
                    newModel.ProjectTasks = nquery.ToList();
                }
        ...
        {
            var _taskList = new List<SprintTask>();
            using (var db = new ScrumApplicationDbContext())
            {
                if(sprintId != 0)
                {
